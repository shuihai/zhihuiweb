    ...
    static function checkAccess() {
        ...
        if( C('USER_AUTH_ON') ){
            ...
            if((!empty($_module['no']) && !in_array(strtoupper(CONTROLLER_NAME),$_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(CONTROLLER_NAME),$_module['yes']))) {
                ...
                //检查当前操作是否需要认证
                if((!empty($_action['no']) && !in_array(strtoupper(CONTROLLER_NAME.'/'.ACTION_NAME),$_action['no'])) || (!empty($_action['yes']) && in_array(strtoupper(CONTROLLER_NAME.'/'.ACTION_NAME),$_action['yes']))) {
                    return true;
                }else {
                    return false;
                }
